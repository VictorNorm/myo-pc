...
export const apiV2Request = async (endpoint, options = {}) => {
  ...
  const url = endpoint.startsWith('/api/v2') ? `${BASE_URL}${endpoint}` : `${API_V2_BASE}${endpoint}`;
...
export const programsAPI = {
  ...
  createWithWorkouts: (programData) => apiV2Post('/programs/create-with-workouts', programData),
  updateStatus: (programId, status) => apiV2Patch(`/programs/${programId}/status`, { status }),
  delete: (programId) => apiV2Delete(`/programs/${programId}`),
  getNextWorkout: (programId) => apiV2Get(`/programs/${programId}/nextWorkout`),
...
export const workoutsAPI = {
  ...
  complete: (workoutData) => apiV2Post('/workouts/completeWorkout', workoutData),
  rateExercise: (exerciseData) => apiV2Post('/workouts/rate-exercise', exerciseData),
  addToProgram: (workoutData) => apiV2Post('/workouts/addworkout', workoutData)
};
...
export const exercisesAPI = {
  ...
  upsertToWorkout: (workoutData) => apiV2Post('/exercises/upsertExercisesToWorkout', workoutData)
};
...
export const muscleGroupsAPI = {
  ...
  update: (id, muscleGroupData) => apiV2Put(`/muscle-groups/${id}`, muscleGroupData),
...
export const statsAPI = {
  getExerciseProgression: (programId) => apiV2Get(`/progression/programs/${programId}/exercises`),
  getVolumeData: (programId, timeframe = 'program') => apiV2Get(`/completed-exercises/programs/${programId}?timeframe=${timeframe}`),
  getWorkoutProgress: (programId, timeframe = 'program') => apiV2Get(`/workout-progress/programs/${programId}?timeframe=${timeframe}`)
};
...
export const templatesAPI = {
  getAll: (filters = {}) => {
    ...
    const endpoint = params.toString() ? `/templates?${params}` : '/templates';
    return apiV2Get(endpoint);
  },
  getById: (id) => apiV2Get(`/templates/${id}`),
  createProgram: (templateId, customName = null) => apiV2Post(`/templates/${templateId}/create-program`, { customName }),
